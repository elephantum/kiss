create table olap_summary_daily_{data_source} (
  p string,
...
location 's3://enter-kiss-test/enter_proto_{data_source}/olap_summary_daily_{data_source}/';
alter table olap_summary_daily_{data_source} recover partitions;


create table olap_summary_daily_normalized_{data_source} (
  p string,
...
partitioned by (`date` string)
location 's3://enter-kiss-test/enter_proto_{data_source}/olap_summary_daily_normalized_{data_source}/';
alter table olap_summary_daily_{data_source} recover partitions;


create table olap_summary_cumulative_{data_source} (
  p string,
...
partitioned by (`date` string)
location 's3://enter-kiss-test/enter_proto_{data_source}/olap_summary_cumulative_{data_source}/';
alter table olap_summary_cumulative_{data_source} recover partitions;
